...
impl Queries {
    ...
    pub(crate) async fn count_pilots(&mut self) -> anyhow::Result<i64> {
        let row = self.client.query_one(COUNT_PILOTS, &[]).await?;
        Ok(sqlc_core::FromPostgresRow::from_row(&row)?)
    }
    pub(crate) async fn delete_pilot(&mut self, id: i32) -> anyhow::Result<()> {
        self.client.execute(DELETE_PILOT, &[&id]).await?;
        Ok(())
    }
    pub(crate) async fn list_pilots(&mut self) -> anyhow::Result<Vec<Pilot>> {
