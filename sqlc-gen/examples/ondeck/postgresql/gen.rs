...
pub(crate) async fn create_city(
    ...
    let row = client.query_one(CREATE_CITY, &arg.as_params()).await?;
    Ok(row)
}
pub(crate) async fn create_venue(
    ...
    let row = client.query_one(CREATE_VENUE, &arg.as_params()).await?;
    Ok(row)
}
...
pub(crate) async fn get_venue(
    ...
    let row = client.query_one(GET_VENUE, &arg.as_params()).await?;
    Ok(row)
}
pub(crate) async fn list_cities(
    ...
    let iter = client.query(LIST_CITIES, &arg.as_params()).await?;
    Ok(iter)
}
...
pub(crate) async fn update_city_name(
    ...
    client.execute(UPDATE_CITY_NAME, &arg.as_params()).await?;
    Ok(())
}
pub(crate) async fn update_venue_name(
    ...
    let row = client.query_one(UPDATE_VENUE_NAME, &arg.as_params()).await?;
    Ok(row)
}
